...
class NewCommand(BaseCommand):
    ...
    def _load_item_classes():
        ...
        from models import ToDoItem, ToBuyItem, ToReadItem, ToFixItem
...
class DeleteCommand(NewCommand):
    @staticmethod
    def label():
        return 'delete'

    def remove_task(self, input_function, objects):
        selection = int(input_function('Input number: '))
        to_remove = objects[selection]
        del objects[selection]
        print('Removed {}'.format(str(to_remove)))
    ...
    def perform(self, objects, *args, **kwargs):
        ...
        input_function = get_input_function()
        self.user_input_secure_wrap(self.remove_task, input_function, objects)
